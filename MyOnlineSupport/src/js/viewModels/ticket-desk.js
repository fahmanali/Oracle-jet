  ...
  function (oj, ko, $) {
    function TicketDeskViewModel() {
      ...
      var ticketListCollection = new oj.Collection(null, {
        url: "https://python-firebase-project2-default-rtdb.firebaseio.com/tickets/tickets.json",
        model: ticketModelItem,
      });
      self.ticketListDataSource(new oj.CollectionTableDataSource(ticketListCollection));
      /* Utils */
      self.formatDate = function (date) {
        var formatDate = oj.Validation.converterFactory(oj.
          ConverterFactory.CONVERTER_TYPE_DATETIME)
          .createConverter(
            {
              'pattern': 'dd/MM/yyyy'
            }
      ...
      self.tabBarDataSource = new oj.ArrayTableDataSource(self.tabData,
        { idAttribute: 'id' });
